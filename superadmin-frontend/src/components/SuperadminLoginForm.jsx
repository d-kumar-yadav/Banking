...
const SuperadminLoginForm = ({ setislogin_superadmin }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
     ...
     if (response.status === 200) {
        ...
        setislogin_superadmin(true);
        localStorage.setItem("setislogin_superadmin", "true");
      ...
      setFormData({ email: '', password: '' });
      localStorage.removeItem("setislogin_superadmin");
      navigate("/Superadmin-login");
    }
  };
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      ...
      <div className="space-y-1.5">
        <label className="text-xs font-bold text-slate-700 uppercase tracking-wide">Superadmin Email</label>
        <div className="relative group">
          ...
          <input
            ...
            placeholder="superadmin@mybank.com"
          />
        </div>
      </div>
